...
if [ "$ac" -eq 1 ]; then
    ...
    if [ "$battery_percent" -gt  98 ]; then
        icon="󰂅"
        	 ...
        	 dunstify -a "battery full " -u critical  -r 9994 -h int:value:"$battery_percent" -i "battery" "Battery" "Currently at $battery_percent%" -t 5000
    ...
    if [ "$battery_percent" -gt 95 ]; then
      ...
      icon="󰂃"
        	 dunstify -a "Low battery " -u critical -r 9994 -h int:value:"$battery_percent" -i "battery" "Battery" "Currently at $battery_percent%" -t 6000
    fi
   echo "$icon $battery_percent%"
fi
